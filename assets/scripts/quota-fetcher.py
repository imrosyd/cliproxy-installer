...
def _load_oauth_credentials():
    """Load OAuth client credentials from env vars or a local credentials file."""
    creds_file = os.path.join(AUTH_DIR, ".credentials")
    client_id     = os.environ.get("CLIPROXY_OAUTH_CLIENT_ID")
    client_secret = os.environ.get("CLIPROXY_OAUTH_CLIENT_SECRET")
...
OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET = _load_oauth_credentials()


def load_auth_files():
    """Load all authentication files, including Copilot, Gemini CLI, Qwen, etc."""
    auth_files = []
    pattern = os.path.join(AUTH_DIR, "*.json")

    for filepath in glob.glob(pattern):
        # Skip package.json or config-like JSONs if they accidentally exist
        if "quota-cache" in filepath:
            continue
        ...
        try:
            with open(filepath, 'r') as f:
                ...
                # Simple validation that this is an auth file
                if 'email' in data or 'username' in data or 'access_token' in data:
                    data['_filepath'] = filepath
                    data['_filename'] = os.path.basename(filepath)
                    
                    # Ensure type is set based on filename if missing
                    base_name = os.path.basename(filepath).split('-')[0]
                    data['type'] = data.get('type', base_name)
...
def fetch_all_quotas():
    ...
    for auth in auth_files:
        email = auth.get('email') or auth.get('username') or 'unknown'
        filename = auth.get('_filename', '')
        auth_type = auth.get('type', 'antigravity')

        if auth_type != 'antigravity':
            print(f"Skipping API fetch for {auth_type} account: {email}")
